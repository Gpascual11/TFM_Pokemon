...
class HeuristicV2Doubles(BaseHeuristic2v2):
    ...
    def _score_switch(self, candidate, current_pokemon, slot, battle, my_status) -> float:
        ...
        max_my_damage = max(
            (calculate_base_damage(m, current_pokemon, opp, my_status)
             for m in slot_moves for opp in opponents),
            default=0.0,
        )
    ...
    def _switch_quality(candidate, battle) -> float:
        opp_types = [
            t for opp in (battle.opponent_active_pokemon or [])
            if opp is not None for t in opp.types
        ]
        if not opp_types:
            return 0.0
