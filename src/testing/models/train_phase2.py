...
def make_env(rank: int, port: int):
    ...
    def _init():
        ...
        opponent = SimpleHeuristicsPlayer(
            ...
            account_configuration=AccountConfiguration(f"SHeur{rank}{unique_suffix}", None),
        )
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Train Phase 2 PPO against SimpleHeuristicPlayer.")
    ...
    parser.add_argument("--resume", action="store_true", help="Resume from ppo_pokemon_baseline.zip")
    ...
    try:
        ...
        print(f"Phase 1 weights loaded. Learning rate overridden to {model.learning_rate}.")
    except FileNotFoundError:
        print(f"Baseline {model_path}.zip not found. Starting from scratch (not recommended for Phase 2).")
        model = MaskablePPO(
