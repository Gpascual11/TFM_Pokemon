...
class EnsemblePlayer(SimpleHeuristicsPlayer):
    ...
    def choose_move(self, battle):
        ...
        combined_logits = (self.alpha * ppo_probs) + ((1 - self.alpha) * heuristic_scores)
...
async def main():
    ...
    def make_eval_env():
        ...
        base_env = PokemonMaskedEnv(
            ...
            account_configuration1=AccountConfiguration(get_unique_name("EvalPPO"), None)
        )
        ...
        dummy_opponent = RandomPlayer(
            server_configuration=server_config,
            account_configuration=AccountConfiguration(get_unique_name("EvalDummy"), None)
        )
    ...
    ppo_player = EnsemblePlayer(
        ...
        alpha=1.0,
        server_configuration=server_config,
        account_configuration=AccountConfiguration(get_unique_name("EvalPPO_Pure"), None)
    )
    ...
    from testing.heuristics.singles.heuristics import HeuristicV1, HeuristicV2, HeuristicV3, HeuristicV4, HeuristicV5, HeuristicV6

    opponents = [
        ("Random", RandomPlayer(
            server_configuration=server_config,
            account_configuration=AccountConfiguration(get_unique_name("OppRand"), None)
        )),
        ("MaxBP", MaxBasePowerPlayer(
            server_configuration=server_config,
            account_configuration=AccountConfiguration(get_unique_name("OppMaxBP"), None)
        )),
        ("poke-env-Heur", SimpleHeuristicsPlayer(
            server_configuration=server_config,
            account_configuration=AccountConfiguration(get_unique_name("OppHeur"), None)
        )),
        ("Custom-V1", HeuristicV1(
            server_configuration=server_config,
            account_configuration=AccountConfiguration(get_unique_name("OppV1"), None)
        )),
        ("Custom-V2", HeuristicV2(
            server_configuration=server_config,
            account_configuration=AccountConfiguration(get_unique_name("OppV2"), None)
        )),
        ("Custom-V3", HeuristicV3(
            server_configuration=server_config,
            account_configuration=AccountConfiguration(get_unique_name("OppV3"), None)
        )),
        ("Custom-V4", HeuristicV4(
            server_configuration=server_config,
            account_configuration=AccountConfiguration(get_unique_name("OppV4"), None)
        )),
        ("Custom-V5", HeuristicV5(
            server_configuration=server_config,
            account_configuration=AccountConfiguration(get_unique_name("OppV5"), None)
        )),
        ("Custom-V6", HeuristicV6(
            server_configuration=server_config,
            account_configuration=AccountConfiguration(get_unique_name("OppV6"), None)
        ))
    ]
    ...
    for opp_name, opp_player in opponents:
        ...
        ensemble_winrate = await run_benchmark(ensemble_player, opp_player, n_battles=100)
        
        results.append([opp_name, f"{ppo_winrate:.1f}%", f"{ensemble_winrate:.1f}%"])

    # Final Output Table
    print("\n" + tabulate(results, headers=["Opponent", "Pure PPO", "Ensemble"], tablefmt="grid"))
