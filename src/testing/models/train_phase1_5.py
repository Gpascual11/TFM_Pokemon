...
def make_env(rank: int, port: int):
    ...
    def _init():
        ...
        opponent = MaxBasePowerPlayer(
            ...
            account_configuration=AccountConfiguration(f"MBP{rank}{unique_suffix}", None),
        )
        ...
        base_env = PokemonMaskedEnv(
            battle_format="gen9randombattle",
            server_configuration=server_config,
            account_configuration1=AccountConfiguration(f"PPOBot{rank}{unique_suffix}", None),
        )
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description="Train Phase 1.5 PPO against MaxBasePowerPlayer.")
    ...
    parser.add_argument("--resume", action="store_true", help="Resume from current phase weight instead of starting from Phase 1")
    ...
    try:
        ...
        model = MaskablePPO.load(load_path, env=vec_env, tensorboard_log="./ppo_pokemon_tensorboard/")
        # Lower LR slightly from 3e-4 to 2e-4 for this intermediate phase
        model.learning_rate = 2e-4
        print(f"Weights loaded successfully. Learning rate set to {model.learning_rate}.")
