...
from ..s01_env.pokemon_env import GauntletEnvWrapper, PokemonMaskedEnv
...
def make_env(rank: int, port: int, is_eval: bool = False):
    ...
    def _init():
        ...
        if rank % 3 == 0:
            ...
            opponent_cls = (
                MaxBasePowerPlayer  # Reinforces defensive survival & type advantage
            )
            opp_name = f"{prefix}MGntlt{rank}{unique_suffix}"
        else:
            opponent_cls = (
                SimpleHeuristicsPlayer  # Reinforces complex tactical pivoting
            )
        ...
        base_env = PokemonMaskedEnv(
            ...
            account_configuration1=AccountConfiguration(
                f"{prefix}GauntBot{rank}{unique_suffix}", None
            ),
        )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Train Phase 3 PPO strictly against the Gauntlet."
    )
    parser.add_argument(
        "--timesteps", type=int, default=2_000_000, help="Total timesteps to train"
    )
    parser.add_argument(
        "--ports", type=int, nargs="+", default=[8000], help="List of server ports"
    )
    parser.add_argument(
        "--eval_ports",
        type=int,
        nargs="+",
        default=[],
        help="Ports dedicated to the evaluation environments",
    )
    args = parser.parse_args()

    num_envs = len(args.ports)
    print("--- Starting Phase 3 Training (The Gauntlet) ---")
    ...
    if args.eval_ports:
        ...
        eval_env = SubprocVecEnv(
            [make_env(i, port, is_eval=True) for i, port in enumerate(args.eval_ports)]
        )
    ...
    try:
        ...
        print(
            f"Phase 2 weights loaded. Learning rate overridden to {model.learning_rate}."
        )
