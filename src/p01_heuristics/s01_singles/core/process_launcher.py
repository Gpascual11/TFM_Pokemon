...
def _worker(
    ...
    # We need 'src' in sys.path so we can import as 'p01_heuristics.s01_singles...'
    _this_dir = os.path.dirname(os.path.abspath(__file__))  # core
    _singles_dir = os.path.dirname(_this_dir)  # s01_singles
    _heuristics_dir = os.path.dirname(_singles_dir)  # p01_heuristics
    ...
    # We must use the full package path so that relative imports inside
    # battle_manager and factory (which go up to ..agents) work.
    _pkg_full = "p01_heuristics.s01_singles.core"
