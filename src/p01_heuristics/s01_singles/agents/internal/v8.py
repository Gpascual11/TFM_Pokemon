...
Builds on V7's boost-aware damage scoring with two validated additions:

1. **Conservative Priority KO**: If a priority move can KO the opponent with
   a wide safety margin (2x overkill), use it to prevent them from attacking.
   Fires rarely (1-2 times per game, not 17 like the broken version).

2. **Opponent OHKO Detection**: If the opponent can likely OHKO us AND we have
   a teammate with a positive matchup, switch out preemptively. This catches
   situations where matchup score alone wouldn't trigger a switch (e.g., we're
   neutral on type but the opponent has a boosted sweeper ready to KO).

Same core: boost-aware damage, smart switch targets, weather/terrain mods.
NO hazards, NO setup — proven to waste turns in random battles.
"""
...
class HeuristicV8(BaseHeuristic1v1):
    """Smart boost-aware attacker: V7 core + priority KO + OHKO detection."""
    ...
    def _pre_move_hook(self, battle):
        """Priority KO: use priority move ONLY if it overkills by 2x margin.

        This fires 1-2 times per game on average. The 2x margin prevents
        the false-positive problem that destroyed earlier versions.
        """
        ...
        for move in priority_moves:
            ...
            # Normalized damage estimate (proportion of max HP)
            # Using simplified level 80 formula: (0.44 * bp * ratio + 2) * mods / ~300hp
            raw = (0.44 * move.base_power * ratio + 2) * stab * effectiveness * expected_hits
            dmg_fraction = raw / 300.0

            # Require 2x overkill — very conservative to avoid false positives
    ...
    def _score_move(self, move, me, opp, physical_ratio, special_ratio, battle, my_speed, opp_speed) -> float:
        ...
        # Priority moves are more valuable when we're slower
    ...
    def _should_switch(self, me, opp, my_status, my_speed, opp_speed, max_score, battle) -> str:
        ...
        # Heavily debuffed defenses
        if me.boosts.get("def", 0) <= -3 or me.boosts.get("spd", 0) <= -3:
            return "matchup"

        # Heavily debuffed offensive stat
        ...
        # V8 addition: opponent likely OHKOs us and we're slower
        if my_speed < opp_speed and me.current_hp_fraction < 0.5:
            opp_threat = self._estimate_opp_threat(me, opp)
            if opp_threat >= me.current_hp_fraction:
                return "matchup"

        return ""

    def _estimate_opp_threat(self, defender, attacker) -> float:
        """Estimate opponent's best STAB damage as fraction of our HP."""
        opp_types = [t for t in attacker.types if t is not None]
        if not opp_types:
            return 0.3

        opp_atk = self._stat_estimation(attacker, "atk")
        opp_spa = self._stat_estimation(attacker, "spa")
        our_def = self._stat_estimation(defender, "def")
        our_spd = self._stat_estimation(defender, "spd")

        best_fraction = 0.0
        for opp_type in opp_types:
            effectiveness = defender.damage_multiplier(opp_type)
            # Assume 80 BP STAB move
            phys = (0.44 * 80 * (opp_atk / max(our_def, 1.0)) + 2) * 1.5 * effectiveness
            spec = (0.44 * 80 * (opp_spa / max(our_spd, 1.0)) + 2) * 1.5 * effectiveness
            fraction = max(phys, spec) / 300.0
