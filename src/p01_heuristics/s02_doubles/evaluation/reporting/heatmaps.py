...
# Configuration
DEFAULT_DATA_DIR = Path("data/benchmarks_doubles_unified")
DEFAULT_OUTPUT = Path("src/p01_heuristics/s02_doubles/evaluation/results/full_heatmap.png")
...
def generate_heatmap(data_dir: Path, output_path: Path):
    ...
    for f in all_files:
        try:
            ...
            # Note: doubles csv might use 'heuristic' or 'version'
            df = df.rename(columns={"version": "agent", "heuristic": "agent", "opponent": "opponent"})
    ...
    plt.title("Unified Double Battles Benchmark", fontsize=16)
