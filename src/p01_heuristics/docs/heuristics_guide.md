...
|--------|-------------------|------------------------|-------------|
| **Internal Heuristics** | `v1`–`v14` | `agents/internal/` (`HeuristicV1`–`HeuristicV14`) | Main research agents; most subclass `BaseHeuristic1v1` (not a single inheritance chain). |
...
- **Logic**: Selects the move with the highest `base power × type effectiveness × STAB`.
- **Weakness**: No switching.
- **Not** the `random` baseline.
...
### V7: Boost-aware attacker

- **Logic**: Uses boosted stats in damage scoring and Abyssal-style matchup switching.
- **Does not** set hazards or setup (those start at V9).

### V8: Priority KO + known immunities

- **Logic**: V7 plus a conservative priority-KO hook and skipping moves the foe is immune to **when the ability is already known**.
...
- **Logic**: Matchup-based fainted/forced switch-in and Gen 9 Terastallization.

### V13: Recovery and revealed-move matchups

- **Logic**: Choice-lock, sweeper reactions, recovery, conservative Tera, matchup damage from **revealed** moves.
...
- **`abyssal`**: Pokechamp AbyssalPlayer.
- **`one_step` / `safe_one_step`**: **Same class** (`SafeOneStepPlayer`). Not two methods.
...
5. Update `AgentFactory.available_internal()` in `p00_core/core/factory.py`.
