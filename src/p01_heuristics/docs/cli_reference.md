# Heuristics & Evaluation CLI Reference (1v1)
...
```bash
bash src/p00_core/scripts/launch_custom_servers.sh 1
```
...
## 2. Parallel benchmark (recommended): `p00_core/engine/benchmark.py`

Runs a full matchup matrix (including self-play) and writes one CSV per matchup:
`data/benchmarks/all_10k/gen9randombattle/{agent}_vs_{opponent}.csv`.
...
- **`--agents <...>`**: primary agents to evaluate. If omitted, runs all registered heuristic (v1-v14) and baseline agents from `AgentFactory` (no LLMs).
- **`--opponents <...>`**: opponent set. If omitted, runs all registered heuristic (v1-v14) and baseline agents from `AgentFactory` (no LLMs).
...
## 3. Single worker batch: `p00_core/engine/worker.py`

Runs one `(agent vs opponent)` batch on a single port and writes a single CSV.
Mostly useful for debugging.

```bash
uv run python src/p00_core/engine/worker.py \
  --agent v6 \
  --opponent random \
  --n-battles 25 \
  --port 8000 \
...
## 4. Debug runner: `p00_core/debug/debug_runner.py`

Runs a few single battles (single-process) and prints turn progress.

```bash
uv run python src/p00_core/debug/debug_runner.py \
...
### `p00_core/engine/run_single.py`

Runs a single matchup using the `core/` pipeline (`BattleManager` / `ProcessLauncher`).
Default output goes to `data/testing/runs/`.

```bash
uv run python src/p00_core/engine/run_single.py v6 random 1000 \
  --ports 8000 8001 8002 8003
```

### `p00_core/engine/serial_benchmark.py`

Runs a heuristic-only round-robin matrix and writes into `--data-dir`.

```bash
uv run python src/p00_core/engine/serial_benchmark.py 1000 \
  --ports 4 \
  --resume
```
...
```bash
uv run python src/p00_core/reporting/plots/generate_full_report.py \
  --data-dir data/benchmarks/all_10k/gen9randombattle
```
...
```bash
uv run python src/p00_core/reporting/plots/generate_report.py \
  --data-dir data/benchmarks/all_10k/gen9randombattle \
  --agent v8
```
