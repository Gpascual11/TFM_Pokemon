...
class MLBaselineAgent(BaseHeuristic1v1):
    ...
    def __init__(self, **kwargs):
        battle_format = kwargs.pop("battle_format", "gen9randombattle")
        super().__init__(**kwargs)
        self.model = xgb.XGBClassifier()
        
        # Resolve the path to the trained JSON model
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "../../../"))
        model_path = os.path.join(project_root, f"src/p02_imitation_learning/s03_training/models/{battle_format}/ml_baseline.json")
