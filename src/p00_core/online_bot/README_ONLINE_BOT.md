...
chmod +x src/p00_core/online_bot/run_online_bot.py
```
...
1. Start the bot:
   ```bash
   uv run python src/p00_core/online_bot/run_online_bot.py --username "YourBotUsername" --password "YourBotPassword" --mode accept
   ```
...
* **Play a batch of 20 games**:
  ```bash
  uv run python src/p00_core/online_bot/run_online_bot.py --username "YourBotUsername" --password "YourBotPassword" --mode ladder --games 20 --agent v13
  ```
