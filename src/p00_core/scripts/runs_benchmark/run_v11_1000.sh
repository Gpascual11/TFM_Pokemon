#!/bin/bash
# run_v11_1000.sh — 1000 games per matchup for V11 against major opponents
# Statistical validation: ±3.1% CI at 95% confidence
...
AGENT="v11"
OPPONENTS="v7 v8 v9 v10 abyssal simple_heuristic random"
N_BATTLES=1000
PORTS=8
CONCURRENCY=20
OUT_BASE="data/testing/backup/benchmarks_v11_1000"
MAX_RETRIES=2
...
trap 'cleanup; avis_telegram "KILLED: V11 1000g benchmark was terminated"; exit 1' SIGTERM SIGINT SIGHUP

TOTAL_START=$(date +%s)
avis_telegram "Starting V11 benchmark: $N_BATTLES games x ${GENS// /, } x $AGENT vs ${OPPONENTS// /, } ($(date '+%H:%M'))"

cleanup

for GEN in $GENS; do
    avis_telegram "[$GEN] Starting V11 matchups"
    START_GEN=$(date +%s)
    ...
    avis_telegram "[$GEN] V11 matchups done (${GEN_MIN}m)"
    cleanup
done
...
avis_telegram "V11 benchmark DONE ($N_BATTLES games). Total: ${TOTAL_HOURS}h${TOTAL_MIN}m. Results in $OUT_BASE/ ($(date '+%H:%M'))"
