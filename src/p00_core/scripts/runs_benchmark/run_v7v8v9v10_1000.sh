#!/bin/bash
# run_v7v8v9v10_1000.sh — 1000 games per matchup for V7, V8, V9, V10
...
PORTS=8
CONCURRENCY=20
OUT_BASE="data/testing/backup/benchmarks_v7v8v9v10_1000"
MAX_RETRIES=2
...
trap 'cleanup; avis_telegram "KILLED: V7-V10 1000g benchmark was terminated"; exit 1' SIGTERM SIGINT SIGHUP

TOTAL_START=$(date +%s)
avis_telegram "Starting V7-V10 benchmark: $N_BATTLES games x ${GENS// /, } x ${AGENTS// /, } ($(date '+%H:%M'))"

cleanup

for GEN in $GENS; do
    avis_telegram "[$GEN] Starting all agents"
    START_GEN=$(date +%s)
    ...
    avis_telegram "[$GEN] All agents done (${GEN_MIN}m)"
    cleanup
done
...
avis_telegram "V7-V10 benchmark DONE ($N_BATTLES games). Total: ${TOTAL_HOURS}h${TOTAL_MIN}m. Results in $OUT_BASE/ ($(date '+%H:%M'))"
