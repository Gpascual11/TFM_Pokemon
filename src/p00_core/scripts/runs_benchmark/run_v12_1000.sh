#!/bin/bash
# run_v12_1000.sh — 1000 games per matchup for V12 against major opponents
...
PORTS=8
CONCURRENCY=20
OUT_BASE="data/testing/backup/benchmarks_v12_1000"
MAX_RETRIES=2
...
trap 'cleanup; avis_telegram "KILLED: V12 1000g benchmark was terminated"; exit 1' SIGTERM SIGINT SIGHUP

TOTAL_START=$(date +%s)
avis_telegram "Starting V12 benchmark: $N_BATTLES games x ${GENS// /, } x $AGENT vs ${OPPONENTS// /, } ($(date '+%H:%M'))"

cleanup

for GEN in $GENS; do
    avis_telegram "[$GEN] Starting V12 matchups"
    START_GEN=$(date +%s)
    ...
    avis_telegram "[$GEN] V12 matchups done (${GEN_MIN}m)"
    cleanup
done
...
avis_telegram "V12 benchmark DONE ($N_BATTLES games). Total: ${TOTAL_HOURS}h${TOTAL_MIN}m. Results in $OUT_BASE/ ($(date '+%H:%M'))"
