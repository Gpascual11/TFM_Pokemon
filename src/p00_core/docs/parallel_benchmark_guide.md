...
2. **Missing Data Analysis (Self-Healing)**:
    - For every matchup (e.g., `v1 vs random`), it checks if `data/benchmarks/all_10k/v1_vs_random.csv` exists (legacy: `data/testing/benchmarks_unified/...` or `data/benchmarks_unified/...`).
...
3. **Server Startup**: It calls `src/p00_core/scripts/launch_custom_servers.sh` to spawn `N` separate Node.js processes for Pokémon Showdown on consecutive ports.
...
- **Combined CSV**: `data/benchmarks/all_10k/` (legacy: `data/testing/benchmarks_unified/` or `data/benchmarks_unified/`)
- **Thinking Logs**: `src/p00_core/results/LLM/`
...
src/p00_core/
├── core/
│   ├── factory.py        <-- Agent Management (v1-v14 + baselines + LLMs)
...
├── docs/
│   ├── csv_schema.md                 <-- 46-column documentation
│   ├── statistical_justification.md  <-- 10k games methodology
│   ├── cli_reference.md              <-- All CLI commands
│   ├── data_layout.md                <-- On-disk structure
│   └── parallel_benchmark_guide.md   <-- THIS FILE
src/p01_heuristics/
├── agents/
│   ├── internal/         <-- V1-V14 heuristic implementations
