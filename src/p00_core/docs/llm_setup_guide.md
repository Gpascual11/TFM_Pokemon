...
├── src/
│   ├── p00_core/        ← unified core utilities and benchmark engine
│   │   ├── core/        ← AgentFactory, process_launcher
│   │   ├── engine/      ← benchmark.py, worker.py
│   │   └── docs/        ← user guides and references
│   └── p01_heuristics/  ← rule-based heuristic agents
└── pyproject.toml
```
...
```sh
uv run python src/p00_core/engine/benchmark.py 100 \
...
| `--out` | `data/benchmarks/all_10k` | Output directory for `{agent}_vs_{opponent}.csv` |
...
1. **Per-matchup CSVs** in `data/benchmarks/all_10k/` — one file per agent-vs-opponent pair
...
- **Light Debugging:** Run a tiny benchmark (or a single worker batch) to validate connectivity and backend settings, e.g.:
  ...
  - `uv run python src/p00_core/engine/benchmark.py 1 --agents pokechamp --opponents random --ports 1 --concurrency 1`
  - Or directly: `uv run python src/p00_core/engine/worker.py --agent pokechamp --opponent random --n-battles 1 --port 8000 --concurrency 1 --out /tmp/test.csv`
