...
DEFAULT_DATA_DIR = _ROOT / "data" / "benchmarks" / "all_10k"
    ...
    try:
        ...
        for i in range(DEFAULT_PORT, DEFAULT_PORT + n_ports):
            ...
            while not ready and (time.time() - start_time) < timeout:
                try:
                    conn = socket.create_connection(("127.0.0.1", i), timeout=1)
                    conn.close()
                    ready = True
                except (ConnectionRefusedError, OSError):
                    await asyncio.sleep(1)
            if not ready:
                print(f"❌ Server on port {i} failed to start in time.")
        print(f"✅ All {n_ports} servers initialized.")
