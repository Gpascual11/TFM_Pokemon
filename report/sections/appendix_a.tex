...
\begin{itemize}
    ...
    \item \textbf{\texttt{safe\_one\_step} / \texttt{one\_step}:} Dues etiquetes històriques de la mateixa classe \texttt{SafeOneStepPlayer}: lookahead determinista d'un pas (dany esperat $\times$ precisió $\times$ STAB) sense cridar \texttt{LocalSim}. El rànquing Bradley--Terry les compta com a dos oponents.
\end{itemize}
...
\input{tables/appendix_agents_catalog}
...
\input{tables/appendix_telemetry_dictionary}
...
Atès que l'acció d'atacar és aproximadament 2,88 vegades més habitual que la de canviar en la distribució empírica humana, s'aplica el paràmetre \texttt{scale\_pos\_weight=2.88}. El fitxer \path{xgboost_advanced_threshold.json} desa diversos punts de la corba precisió--record (F1 i igualació de freqüència); l'agent carrega en inferència el llindar recomanat $\tau = 0{,}5525$ (el mateix valor de la Taula~\ref{tab:app_hyperparameters_il} i del Capítol~\ref{chap:implementacio}).
...
\begin{equation}
    r_t = R_{\mathrm{base}}(s_t, a_t, s_{t+1}) + F(s_t, s_{t+1}) - 0{,}02
...
La funció de conformatge basada en potencial es computa com la diferència temporal $F(s_t, s_{t+1}) = \gamma \Phi(s_{t+1}) - \Phi(s_t)$ (amb el factor de descompte $\gamma = 0{,}99$, Taula~\ref{tab:app_hyperparameters_ppo}), garantint que no s'alteri la política òptima del procés de decisió subjacent d'acord amb el teorema d'invariància de Ng et al.~\cite{ng_reward_shaping}. El cost de pas $-0{,}02$ penalitza la inacció i incentiva l'agent a cloure la partida de manera resolutiva.
...
\input{tables/appendix_hyperparameters}
