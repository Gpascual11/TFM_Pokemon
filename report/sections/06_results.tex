...
En la mateixa telemetria de matriu, utilitza la Teracristal·lització \textbf{0,96 vegades per partida}, registra només \textbf{0,01} accions de reserva i conserva \textbf{1,84} Pokémon, davant d'1,44 a \texttt{v11}. El canvi de comportament és substancial.
...
\begin{figure}[H]
    ...
    \caption{Enfrontaments de referència de \texttt{v12}, \texttt{v13} i \texttt{v14}. Cada color és l'agent avaluat. Les barres contra MCTS tenen $n=1.000$ i la resta, $n=10.000$. \texttt{v12} domina els agents de referència; \texttt{v13} és més fort contra cerca i \texttt{v21}.}
    \label{fig:heur_key}
\end{figure}
...
Aquest patró és comparable al de \texttt{v21}, que consulta XGBoost en aproximadament el 15\% dels torns, i al de les variants MCTS. Les partides perdudes presenten una discrepància mitjana superior (taxa de \texttt{search\_diff} per torn); és una associació, no una prova causal.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\linewidth]{images/minimax/minimax_override.png}
    \caption{Taxa de discrepància respecte de \texttt{v14} en els torns en què s'executa el maximin. \texttt{v15}/\texttt{v16} discrepen aproximadament en el 67\% dels casos; \texttt{v17}, amb prior $+0{,}15$, en el 37\%.}
...
\begin{figure}[H]
    ...
    \caption{Enfrontaments de referència del minimax d'un torn. Cada color és l'agent avaluat. Les barres contra MCTS tenen $n=1.000$. \texttt{v17} millora respecte de les altres variants, però no supera el 50\% contra \texttt{v14}.}
...
\begin{figure}[H]
    ...
    \caption{Taxa de discrepància respecte de \texttt{v14} entre les decisions de cerca. UCB1 (\texttt{v18}/\texttt{v19}) discrepa aproximadament en el 71\% dels casos i PUCT (\texttt{v20}), en el 19\%.}
...
\begin{figure}[H]
    ...
    \caption{Enfrontaments discriminants d'MCTS. Cada color és l'agent avaluat i totes les barres tenen $n=1.000$. \texttt{v20} s'acosta a Abyssal (49\%) i no supera \texttt{v12} (42,7\%).}
    \label{fig:mcts_key}
\end{figure}
...
\begin{figure}[H]
    ...
    \caption{Indicadors tàctics de \texttt{v21} i \texttt{v22}. \texttt{v22} utilitza la Teracristal·lització gairebé una vegada per partida, una freqüència semblant a la de \texttt{v12} a la Figura~\ref{fig:heur_fingerprint}, però obté un resultat molt inferior; la freqüència no mesura la qualitat de la decisió.}
    \label{fig:il_tac}
\end{figure}
