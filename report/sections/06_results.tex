...
28 agents, cadascun com a \emph{nosaltres} contra 28 oponents, format \texttt{gen9randombattle}. Fitxers dirigits \texttt{A\_vs\_B.csv}. PPO es llegeix a part (Secció~\ref{sec:res_ppo}).
...
Cada cel·la és una proporció binomial. Els intervals són Wilson al 95\% (Equació~\ref{eq:wilson_ci}); els tres pisos mostrals són els de la Taula~\ref{tab:ci_n}. A $n=10.000$ el marge màxim és de l'ordre d'$\pm 1$~pp; a $n=1.000$ (totes les cel·les MCTS), $\pm 3$~pp. La Pokédex no entra a $\mathrm{Var}(\hat p)$: s'estima una taxa marginal sota el generador d'equips de la Generació~IX.
...
La pregunta de disseny ---si un maximin d'un torn corregeix \texttt{v14}--- és la de la Secció~\ref{sec:disseny_agents}. Aquí es mesura amb quina freqüència s'executa el mòdul i quan la seva acció difereix de la proposta heurística.
...
\textbf{Totes les cel·les d'aquesta secció tenen $n=1.000$} ($\pm 3$~pp, Taula~\ref{tab:ci_n}). La mitjana global de les files MCTS també té una ponderació diferent de la de les files no MCTS. La pregunta de disseny ---si cinc torns simulats superen l'horitzó de \texttt{v14}--- és la de la Secció~\ref{sec:disseny_agents}.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.92\linewidth]{images/mcts/mcts_future_proxies.png}
...
Les dues variants comparteixen el classificador XGBoost i el llindar $\tau=0{,}5525$ del Capítol~\ref{chap:implementacio}, però difereixen en la selecció de l'acció concreta.
...
\begin{enumerate}
    ...
    \item \textbf{Absència d'intervals de confiança en l'ajust MLE:} Les puntuacions reportades són estimacions puntuals del vector de coeficients de la regressió logística. La proximitat entre valors arrodonits (com 1755 entre \texttt{v14} i Abyssal, o 1730 entre \texttt{v9} i \texttt{v21}) no autoritza a proclamar superioritat estadística d'un agent sobre l'altre.
...
La Taula~\ref{tab:ppo_curriculum_eval} i la Figura~\ref{fig:ppo_curriculum_curves} separen artefactes d'entrenament (Capítol~\ref{chap:implementacio}) i validació congelada. Les dues primeres etapes compleixen el criteri operatiu; la fase~2 no, i la fase~3 s'executa com a prova de continuació.

\begin{table}[htbp]
    \centering
    \caption{Validació congelada del currículum PPO. Els artefactes i hiperparàmetres són la Taula~\ref{tab:ppo_curriculum_impl}.}
    \label{tab:ppo_curriculum_eval}
    \small
    \renewcommand{\arraystretch}{1.2}
    \begin{tabularx}{\textwidth}{clcc>{\raggedright\arraybackslash}X}
        \toprule
        \textbf{Fase} & \textbf{Oponent} & $n$ & \textbf{TV} & \textbf{Porta} \\
        \midrule
        1   & Random       & 1.000  & 91,8\% & $\ge 90\%$: superada \\
        ...
        \arrayrulecolor{gray!30}\hline\arrayrulecolor{black}
        3   & \texttt{v12} & 10.000 & 34,1\% & mesura final (model A) \\
        \bottomrule
    \end{tabularx}
\end{table}
...
\begin{table}[htbp]
    ...
    \begin{tabularx}{\textwidth}{lXccc}
        ...
        \midrule
        \texttt{v17} & maximin d'un torn & 16--18\% & 37\% & 40,94\% \\
        \arrayrulecolor{gray!30}\hline\arrayrulecolor{black}
        \texttt{v20} & PUCT, prior $0{,}70$ & 16--18\% & 18,7\% & 42,7\% \\
        \arrayrulecolor{gray!30}\hline\arrayrulecolor{black}
